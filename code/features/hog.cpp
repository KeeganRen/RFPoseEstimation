...
mxArray *hog(double* im, const mwSize* im_dims, int sbin) 
{    
    // memory for caching orientation histograms & their norms
    int blocks[2];
    blocks[0] = (int)round((double)im_dims[0]/(double)sbin);
    blocks[1] = (int)round((double)im_dims[1]/(double)sbin);
    ...
    for (int x = 1; x < visible[1]-1; x++) {
        for (int y = 1; y < visible[0]-1; y++) {
            // first color channel
            double *s = im + min(x, im_dims[1]-2)*im_dims[0] + min(y, im_dims[0]-2);
            double dy = *(s+1) - *(s-1);
            double dx = *(s+im_dims[0]) - *(s-im_dims[0]);
            double v = dx*dx + dy*dy;
            
            // second color channel
            s += im_dims[0]*im_dims[1];
            double dy2 = *(s+1) - *(s-1);
            double dx2 = *(s+im_dims[0]) - *(s-im_dims[0]);
            double v2 = dx2*dx2 + dy2*dy2;
            
            // third color channel
            s += im_dims[0]*im_dims[1];
            double dy3 = *(s+1) - *(s-1);
            double dx3 = *(s+im_dims[0]) - *(s-im_dims[0]);
